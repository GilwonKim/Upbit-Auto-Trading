    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price and current_price < predicted_close_price:
                ...
                print("krw")
                #volatility = get_volatility("KRW-XRP")
                #print("volatility")
                if krw > 5000:
                    buy_result = upbit.buy_market_order("KRW-XRP", coins * 0.9995) #TargetVolatility/volatility/NumOfCoins*krw*0.9995,
                    post_message(myToken, "#breakthru", "XRP buy : " +str(buy_result))
        else:
            xrp = get_balance("XRP")
            if xrp > 5000/get_current_price("KRW-XRP"):
                sell_result = upbit.sell_market_order("KRW-XRP", xrp)
                post_message(myToken, "#breakthru", "XRP sell : " +str(sell_result))
        time.sleep(1)
